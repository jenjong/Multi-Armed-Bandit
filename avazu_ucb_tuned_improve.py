...
#%%
@jit
def ucb_tuned_improve_df(n_iterations, rounds):
    ...
    p = np.mean(main_df[:,0].astype(int))
    epsilon = np.sqrt(p * (1-p) / len(main_df)) * p
    ...
    for iteration in tqdm(range(n_iterations)):
        ...
        for n_rounds in range(n_items, rounds):
            ...
            if (abs(results[-1]['avg_reward'] - results[-2]['avg_reward']) > epsilon) or (results[-1]['avg_reward'] == 0) or (results[-1]['avg_reward'] == 1):
...
#%%
ucb_tuned_improve_results = pd.DataFrame(ucb_tuned_improve_df(n_iterations=10,
                                                                rounds=100000))

ucb_tuned_improve_results.to_csv('/Users/huiwon/Desktop/Recommendation/bandit/experiment/final_experiment/rounds/avazu_ucb_tuned_improve.csv')
