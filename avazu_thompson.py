...
#%%
@jit
def thompson_df(n_iterations, rounds):
    ...
    for iteration in tqdm(range(n_iterations)):
        ...
        alphas = np.ones(n_items)
        betas = np.ones(n_items)
        
        regret = 0
        
        total_reward = 0
        
        for n_rounds in range(rounds):
...
#%%
thompson_results = pd.DataFrame(thompson_df(n_iterations=10,
                                            rounds=100000))

thompson_results.to_csv('/Users/huiwon/Desktop/Recommendation/bandit/experiment/final_experiment/rounds/avazu_thompson.csv')
